...
void ui_init(void) {
    // 初始化 U8G2
    u8g2_Setup_ssd1306_i2c_64x32_1f_f(&u8g2, U8G2_R0, u8x8_byte_hw_i2c, u8x8_gpio_and_delay);
    ...
    u8g2_DrawStr(&u8g2, 0, 10, " U  00.00V ");
    u8g2_DrawStr(&u8g2, 0, 20, "SET --.--V ");
    u8g2_DrawStr(&u8g2, 0, 30, "SET --.--A ");
...
void ui_update_vbus(uint16_t vbus_mv) {
    char buf[16];
    snprintf(buf, sizeof(buf), " U  %05.2fV ", vbus_mv / 1000.0f);
...
void ui_update_pd_pos(uint8_t pos) {
    ...
    char buf[16];
    snprintf(buf, sizeof(buf), "SET %05.2fV ", pdo->fixed.voltage / 1000.0f);
    u8g2_DrawStr(&u8g2, 0, 20, buf);
    snprintf(buf, sizeof(buf), "SET %05.2fA ", pdo->fixed.current / 1000.0f);
